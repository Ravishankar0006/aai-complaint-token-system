...
export function TechnicianPanel({ techId }: TechnicianPanelProps) {
  ...
  const handleToggleLeave = () => {
    ...
    const res = toggleTechnicianLeave(techId, techStatus.currentStatus);
    if (res.success) {
      loadData();
    }
  };

  const handleStartWork = (tokenId: string) => {
    if (!techUser) return;
    const res = updateTokenStatus(tokenId, "IN_PROGRESS", techUser.id, techUser.name, "technician");
    if (res.success) {
      loadData();
    }
  };

  const handlePutOnHoldSubmit = () => {
          ...
          >
            {activeQueue.length === 0 ? (
              ...
              activeQueue.map((token) => (
                <div
                  key={token.id}
                  className="bg-white dark:bg-[#0b0f19] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 shadow-md flex flex-col gap-4"
                >
                  {/* Top bar info */}
                  ...
                  {token.photoUrl && (
                    <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/60 p-2 border border-slate-200 dark:border-slate-800 rounded-xl w-fit">
                      ...
                      <a href={token.photoUrl} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-blue-500 hover:underline">
                  ...
                  <div className="border-t border-slate-100 dark:border-slate-800/80 pt-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      {token.status === "ASSIGNED" && (
                        <button
                          onClick={() => handleStartWork(token.id)}
                      ...
                      {token.status === "IN_PROGRESS" && (
                        <>
                          <button
                            onClick={() => setShowHoldModal(token)}
                          ...
                          <button
                            onClick={() => setShowResolveModal(token)}
          ...
          >
            {historyQueue.length === 0 ? (
              ...
              historyQueue.map((token) => (
                <div
                  key={token.id}
                  className="bg-white dark:bg-[#0b0f19] border border-slate-200 dark:border-slate-800/80 rounded-xl p-4 shadow-sm flex flex-col gap-2.5 opacity-80"
                >
